...
def exists_blog_post(dbinfo, conn, link):
    sql = ('select blog_post_id '
           'from blog_post '
           'where link = %s;')
    cur = conn.cursor()
...
def find_blog_roll(dbinfo, conn, blog_id, link):
    sql = ('select blog_roll_id '
           'from blog_roll '
...
def store_blog_post_links(dbinfo, conn, blog_post_id, links):
...
#        print('link: ' + link)
        if len(link) > 2000:
            ...
            continue
        if exists_blog_post_link(dbinfo, conn, blog_post_id, link):
            continue
        ...
        cur.close()
        # Store in blog_post_blog_link
        store_blog_post_blog_link(dbinfo, conn, blog_post_id, link)
    conn.commit()

def store_blog_roll_blog_link(dbinfo, conn, blog_roll_id, link):
    ...
    if blog_id is not None:
        if ( exists_blog_roll_blog_link(dbinfo, conn, blog_roll_id, blog_id) ==
                False ):
...
def store_blog_roll(dbinfo, conn, blog_id, blog_roll):
    """
    ...
    for t in blog_roll:
        (link, title) = t
        blog_roll_id = find_blog_roll(dbinfo, conn, blog_id, link)
        if blog_roll_id is None:
            ...
            cur.close()
            blog_roll_id = r[0]
        store_blog_roll_blog_link(dbinfo, conn, blog_roll_id, link)
    conn.commit()

def store_blog_posts(dbinfo, conn, blog_id, rss_entries):
    """
    ...
    for entry in rss_entries:
        ...
        if exists_blog_post(dbinfo, conn, link):
            continue
...
#        print(json.dumps(entry, sort_keys = True, indent = 4))
        ...
        if blog_post_id is not None and content is not None:
            ...
            store_blog_post_links(dbinfo, conn, blog_post_id, links)
    conn.commit()

class RetrieveSiteThread(Thread):
    def __init__(self, dbinfo, blog):
...
def retrieve_blog_site_data(dbinfo):
    ...
    for blog in blogs:
        ...
        # Start a new thread
        th = RetrieveSiteThread(dbinfo, blog)
        th.setName(str(blog))
        th.start()
        threads.append(th)
    print('exit for loop')
